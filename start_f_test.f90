...
!!!!!!!!!!!!!!!!!!!!!!                                     ! open file for seting a population if we are in TEST SET.
  ...
  write(*,*)'farxivread',supereplica,replica,arxiv ! printdebug
!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!! WARNING. MANUAL IMPLEMENTATION !!!!!!!!!!!!!!!!!!!!!!! WARNING. MANUAL IMPLEMENTATION !!!!!!!!
  ...
  if(training.eq.1)then                                                 ! creating random population in t=0 in training phase
    ...
    do iii=1,ind(i)%ngs
      ...
      write(*,*)supereplica,replica,i,iii,'fW', ind(i)%w(iii,1:ind(i)%ngs)  ! printdebug
    end do
    do iii=1,ind(i)%ngs
      read(9000,*) ind(i)%ww(iii,1:ind(i)%ngs)                           ! uploading population Ww from file
      write(*,*)supereplica,replica,i,iii,'fWW',ind(i)%ww(iii,1:ind(i)%ngs)  ! printdebug
    end do
  end if
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
end do
...
if(training.ne.1)then                                                   ! Test set.
  do iii=1,ind(1)%ngs
    ...
    write(*,*)supereplica,replica,iii,'fMZ',ind(1)%MZ(iii,1:pd) ! printdebug
  end do
  do iii=1,ind(1)%ngs
    read(9000,*) ind(1)%MZZ(iii,1:pd)                                   ! uploading MZZ for the whole population from file
    write(*,*)supereplica,replica,iii,'fMZZ',ind(1)%MZZ(iii,1:pd) ! printdebug
  end do
  do iii=1,p
